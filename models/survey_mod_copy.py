...
class Indicateur(models.Model):
	...
	_rec_name = 'indicateur'
	name=fields.Char('Indicateur Name', required=True, translate=True)
	valeur=fields.Float("Valeur de l'indicateur", default=0.0)
	parent_indicateur=fields.Many2one('survey.indicateur', string='Indicateur Parent', ondelete='NULL')
	questions = fields.One2many('survey.question', 'indicateur_id', string='Questions', copy=True)
	def calcul_valeur_indicateur(self,id_indicateur):
		count=0
		somme=0
		indicateurs=self.env['survey.indicateur'].browse(id_indicateur)
		for indicateur in indicateurs:
	    	for question in indicateur.questions:
	    		somme+=question.question_value
	    		count=count+1
	    	indicateur.valeur=somme/count
...
class SurveyQuestion(models.Model):
    ...
    def update_related_indicator(self,question_id):
    	...
    	for question in questions:
	    	...
	    	self.env['survey.indicateur'].browse([question.indicateur_id]).calcul_valeur_indicateur(self,question.indicateur_id)
...
class SurveyUserInputLine(models.Model):
    ...
    def save_line_numerical_box(self, user_input_id, question, post, answer_tag):
        ...
        self.env['survey.question'].browse([vals.question.id]).update_related_indicator(self,question.id)
        return True
